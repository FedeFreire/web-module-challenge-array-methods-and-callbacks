...
function getWinnersByYear(data,finalsCB,yearsCB,winnersCB) {
    let countries = winnersCB(data,finalsCB)
    let years = yearsCB(data,finalsCB)
    let text = []
    for(let x in countries){
       text.push(`In ${years[x]}, ${countries[x]} won the world cup!`)
    }
    return text
}
...
function getAverageGoals(finalsCB) {
    let home = (finalsCB.reduce((a,b) => {
        return a + b["Home Team Goals"]+ b["Away Team Goals"]},0)/finalsCB.length).toFixed(2)
